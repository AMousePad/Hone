...
export function createChipInput(opts: ChipInputOptions): ChipInputHandle {
  ...
  /** Closure variable beats stuffing ids into `dataTransfer`. */
    ...
    for (let i = 0; i < state.promptIds.length; i++) {
      ...
      if (!state.readOnly) {
        ...
        chip.addEventListener("drop", (e) => {
          e.preventDefault();
          chip.classList.remove("hone-chip--drop-target");
          if (draggedChipIndex === null || draggedChipIndex === i) return;
          commitReorder(draggedChipIndex, i);
        });
        ...
        chip.addEventListener("touchend", (e) => {
          ...
          if (targetChip && targetChip.dataset.index !== undefined) {
            ...
            if (draggedChipIndex !== null && draggedChipIndex !== targetIndex) {
              commitReorder(draggedChipIndex, targetIndex);
            }
          }
          draggedChipIndex = null;
        });
