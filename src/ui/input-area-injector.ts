...
export function createInputAreaInjector(
  ...
  function rescan() {
    const ia = findInputArea();
    if (!ia) return;
    if (ia !== attachedTo) attach(ia);
    const bar = findActionBar(ia);
    if (bar) injectInto(bar);
    attachInputListener();
    // `rescan` fires on every active-chat push, which is also where
    // isReady() flips true during the handshake. Without this refresh,
    // the button would stay greyed until the next unrelated event.
    updateAllButtonStates();
  }

  function attach(ia: Element) {
    detachObserver();
    attachedTo = ia;
    ...
    observer.observe(ia, { childList: true, subtree: true });
    const bar = findActionBar(ia);
    if (bar) injectInto(bar);
    attachInputListener();
  }
  ...
  // InputArea may not be mounted yet; poll briefly for it.
  const initial = findInputArea();
  if (initial) {
    attach(initial);
  } else {
